...
- **Checks extended.** `scripts/render-check.cjs` now renders the new section and asserts its required
  ...
  **156 → 160**, all passing (159 pass + 1 intentional skip). Lint clean. Irregularities 80 → **81**.
...
1. **Confirm the #81 fix on a real runner (P0, do this first).** Everything above was proven against a
   mock, because this sandbox has no egress to the exchange. The next scheduled `daily-collection`
   run (12:30 UTC) is the real test. Check: the step finishes **inside 30 min**, `meta-<today>.json`
   exists with `runtime.complete: true`, and `settled-2026-seed.json` finally advances past
   `2026-09-19T04:56:27.917Z` / 400 bars. If it truncates, raise `--concurrency` to 8–10 (still inside
   the documented ceiling) before touching the timeout.
2. **Backfill the 8,950 missing candle seeds.** The seed has been capped at 400 bars since 9/19 and
   tops up by at most `--max-candles` (400) per run, so a full backfill is ~22 runs. Consider a
   one-off `workflow_dispatch` with a raised `--max-candles` and `--budget-minutes`, or accept the
   gradual fill — but **decide and record it**, because the R3 calibration sample size depends on it.
...
- Sandbox has **no general egress**: node `fetch` and `curl` fail TLS to the exchange host, so live
  collection runs only in GitHub Actions. The #81 fix is therefore **verified against a mock, not
  against the exchange** — item 1 above is what closes that gap.
...
- The **settled-2026 candle seed is three days stale and 8,950 markets short** until item 1/2 land;
  `data/calibration-2026.json` scores only the 400 markets that have bars.
