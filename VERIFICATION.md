...
| `https://iem.isu.edu/` + IEM 2026 prospectus PDF → fetch failed (proxy) | Entry N21 marked `verified-via-search` with the university-hosted prospectus as evidence; re-verify directly on a networked run. **UPDATE 2026-09-19: root cause found — the domain itself was wrong (ISU ≠ University of Iowa). Correct properties fetched live (`iem.uiowa.edu/iem/`, `iemweb.biz.uiowa.edu/markets/`); entry N21 upgraded to `verified`, URL corrected. IRREGULARITIES #26.** |

## 7 · 2026-09-19 second batch — 20 new master-list entries (line-by-line, per user request "search 20 new entries … verify no hallucinations before adding")
