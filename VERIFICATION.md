...
## 16. Session 11 — 2026-09-21 batch3, P0 canvass machinery + 20 new sources (249 total)

Branch `arena/01a0c626-elections`. Baseline: 229 sources, 69 irregularities, 40 poll entries, 109 tests.
Task: P0 post-Nov-3 canvass scoring machinery; ingest first poll rows from new pollsters; 20 new verified entries; PR + merge.

### 16a. P0 canvass machinery (post-November 3 scoring path)

- `src/canvass.js`: `detectCertification` (refuses to treat "100% precincts reporting" as certification),
  `parseContestResults` (header-anchored block parse; refuses header-miss, nominee-missing, duplicate names),
  exported `nameInLine` (exact whole-token consecutive-run match; near-misses like "Mike Collin" refuse),
  `reconcile` (majority-runoff -> runoff when winner <= 50%; RCV and exact ties refused; third-party rows
  forced y=0 under `dem-win`), `chamberComposition` (expected-seats guard; majority = floor(n/2)+1; VP
  tiebreak recorded), `buildOutcomeRecord` (sha256 source hash; `certifiedOn` never invented),
  `ingestCanvass`. 9 dedicated tests in `test/canvass.test.mjs`.
- `data/crosslayer/canvass-jurisdictions.json`: 32 jurisdictions generated by
  `scripts/gen-canvass-config.mjs` (idempotent; preserves human-filled fields). Per-jurisdiction
  `authorityId` + `authorityStatus`/`authorityNote` (honest promote-gate text; MA stays
  verified-via-search because its root 403s, so the promote-gate refuses MA by design). majorityRule:
  GA/LA majority-runoff, AK rcv-instant-runoff, rest plurality. AK/DE `nomineesPending`.
  `controlQuestion.resolutionRule.confirmed=false` until the Kalshi CONTROLS-2026 rules page is read.
- `scripts/ingest-canvass.mjs`: default ingest writes staging + per-run evidence
  (`data/crosslayer/canvass/<runId>/<id>.json`); `--set-url` / `--set-certified-on` persist human
  corrections; `--promote` merges dated staged rows through `validateOutcome`. First run 2026-09-21:
  32 jurisdictions, 0 staged (`no-url-yet` by design — no results exist before Nov 3).
- Both workflows gained a "Certified-canvass ingestion (P0)" step (gen-canvass-config + ingest-canvass,
  continue-on-error, 20m timeout on daily); probe `paths:` extended to the canvass files; shared
  concurrency group.

### 16b. Poll layer — first rows from a newly admitted pollster

Five Rasmussen Reports rows (fieldwork Pulse Opinion Research) ingested from direct release fetches:
GA governor, GA senate, SC senate, AK senate (stateRaces) + generic ballot. Every row carries
`methodFamily: ivr-rdd-online-panel-blend` (blended IVR/RDD/online panel — "do not pool unlabeled").
The SC release's biographical framing is flagged as irregularity #69 (publisher's own framing,
uncorroborated; candidate identity corroborated against the exchange). Poll layer: 35 -> 40 entries.
Still pending (not guessed): echelon-2026-04-fl, prri-2026-ava-midterms, kff-2026-06-mifepristone-midterms,
surveyusa-28000-mn (metadata-only topline).

### 16c. New source admissions — 20, all direct-fetch verified (batch3)

`data/sources/admissions-2026-09-21-batch3.json` + `data/probes/session-review-2026-09-21-s11.json`
record URL, observed text and decision per candidate. Master 229 -> **249**.

| New registry id | Official page directly read | Scope verified |
|---|---|---|
| denver-clerk-recorder | denvervotes.org -> denvergov.org Elections Division | Nov 3 2026 midterm + council vacancy special; results at denvergov.org/electionresults |
| salt-lake-county-clerk | slco.org/clerk/elections/ (saltlakecounty.gov) | UT's largest county; 2026 elections home + results links |
| baltimore-city-boe | elections.baltimorecity.gov -> baltimorecity.gov/boe | Director Clifford Tatum; 2026 Election Information; ballot questions; 417 E. Fayette St |
| multnomah-county-elections | multco.us/departments/multnomah-county-elections-division | Nov 3 2026 general; vote-by-mail, RCV, drop sites; signature-challenge letters |
| clark-county-wa-elections | clark.wa.gov/elections/ | "November 3, 2026 General & Special Election"; drop sites; audits; archive |
| ap-norc | apnorc.org | Active 2026 center (State of the Facts 2026; AP VoteCast operator) |
| rutgers-eagleton-poll | eagletonpoll.rutgers.edu (+ eagleton.rutgers.edu) | Garden State Panel (probability); fills NJ gap left by Monmouth's Mar 2025 closure |
| schar-school-gmu | schar.gmu.edu + WaPo PDF | WaPo-Schar 2026 series: MI Sep 10-14 n=803 LV (El-Sayed 48/Rogers 45; Benson 53/James 41); VA Mar n=1,101 RV |
| usc-cesr | dornsife.usc.edu/cesr/ | Understanding America Study probability panel; active 2026 calendar |
| quantus-insights | polls.quantusinsights.org | 2026 archive: national approval x4; ME/TX/NC senate; SC/LA/GA/KY primaries with n per release |
| insideradvantage | insideradvantage.com | Sep 16-18 2026 surveys 1200 LV mixed text/panel: NC/MI/NH senate + TX/SC/OH; Towery retiring Nov |
| cygnal | cygn.al | Generic ballot D 51-42 stat card; GOP-aligned commercial firm -> **needs-review** |
| change-research | changeresearch.com/accuracy-methodology/ | Dynamic Online Sampling; Dem-aligned commercial firm -> **needs-review** |
| wmur | wmur.com/politics | NH Conversation with the Candidate forums; CloseUp; PolitiFact NH |
| nevada-independent | thenevadaindependent.com | Elections hub /elections/2026/general; fact checks; donor transparency |
| minnesota-star-tribune | startribune.com/news-politics/politics | "Peggy Flanagan ... Democratic nominee for Senate" — corroborates SENATEMN-26 nominee |
| alaska-beacon | alaskabeacon.com | States Newsroom AK; Sep 21 2026 senate/gubernatorial coverage |
| vpap | vpap.org | Free nonprofit VA election data; Nov 2026 early-voting dashboard |
| fairvote | fairvote.org | RCV research; corroborates AK two-Sullivans ballot + SC vacancy/runoff (feeds #69 note) |
| vote411-lwv | vote411.org | LWV Education Fund voter platform; hotlines; state voting-rules comparison |

Not admitted (recorded, not fabricated): Bexar + Tarrant (JS shells on two attempts; queued as
render:true probe targets), Hennepin (header-only render; queued), Daily Kos Elections (HTTP 500 x2),
Trafalgar (domain parked/for-sale -> replaced by WaPo-Schar), Pulse Opinion Research (HTTP 500 x2),
Mason-Dixon (no own-domain archive), Baldwin Wallace CRI (PDF assets only), Iowa Hawkeye Poll
(redirects to CSSI), WV MetroNews (no elections hub).

Master 249 total; categories now: Government federal 22, state & local **94**, academic **28**,
pollsters **39**, news **30**, prediction markets 10, ratings/forecasts/analysis **21**, official
publishers 4, contest/methodology 1. `oregon-sos` promoted verified-via-search -> **verified**
(direct re-fetch; dated note in its `verified` field; MA intentionally unchanged).

### 16d. Verification evidence after the batch

`npm run lint` -> `checked 120 data JSON files, 249 sources, 4 outcomes, 5 markets, 69 irregularities
(md rows 69), 40 poll entries — lint: all verified-data provenance checks pass`.
`npm test` -> **109 tests, 108 pass, 1 skip (network), 0 fail**. Updated: site-sources session-5
assertion (oregon-sos no longer verified-via-search), verification-gates total-for-date 40 -> 60
(+ batch3 evidence block), session-9 polls test scoped to its four ids + new Rasmussen methodFamily block.
`npm run pipeline` -> site bundle regenerated (249 sources), all sections render-checked.

### 16e. Three passes

- Pass 1 (implement + verify): canvass module/CLI/config + 20 fetch-verified admissions + poll rows.
- Pass 2 (review/bugs): regex-flag compile bug fixed (patterns always `new RegExp(p,'i')`);
  near-miss nominee matching hardened (`nameInLine`); four stale test count assertions updated to
