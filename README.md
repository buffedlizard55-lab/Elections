...
| Checks | `npm test` (88 tests) + `npm run lint` (provenance, irregularities md⇄json sync, poll-layer tickers) + `scripts/render-check.cjs` (headless site render) | `python scripts/validate_sources.py` (schema + CSV + live checks) |
...
Every one of the 169 entries carries a `category` (the site groups and filters by it) and a `status`.
