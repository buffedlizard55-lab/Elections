...
| `VERIFICATION.md` | Line-by-line audit log for both capture sessions (incl. 2026-09-19 §6: the 21 new entries, URL corrections, and fetch failures) |
...
| `data/irregularities.json` | 15 flagged irregularities/discrepancies with severities and actions |
...
1. **Every value traces to a captured URL.** Every data file carries `capturedFrom` + `capturedAt`
   (and the lint *fails* if one is missing). Both capture sessions' line-by-line notes — 2026-09-18
   (32 sources, Kalshi capture) and 2026-09-19 (21 new sources, 3 new irregularities) — are in
...
the Iowa Electronic Markets entry (53rd source) was verified via live search because `iem.isu.edu`
could not be fetched directly in the sandbox (marked `verified-via-search`, re-check on a networked
run). Full list: [ROADMAP.md](ROADMAP.md).
